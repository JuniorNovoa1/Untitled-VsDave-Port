function onCreate()
    ...
    if not lowQuality and string.lower(songName) == "splitathon" then addLuaSprite('picnic', false) end

    local daBag = 'backgrounds/farm/cornbag';
    if getRandomInt(0, 100) == 0 then daBag = 'backgrounds/farm/popeye'; end
    makeLuaSprite('cornBag', daBag, 1200, 550)
    if not lowQuality then addLuaSprite('cornBag', false) end

    makeLuaSprite('sign', 'backgrounds/farm/sign', 0, 350)
    if not lowQuality then addLuaSprite('sign', false) end
